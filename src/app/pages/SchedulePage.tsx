...
function getLanguageShiftMs(
  language: keyof typeof timelineLocaleByLang,
): number {
...
function formatTimelineRowLabel(
  timestamp: number,
  language: keyof typeof timelineLocaleByLang,
) {
...
function calcTimelineLeftPercent(
  startHour: number,
...
function ActivityThumbnail({
  ...
  return (
    <img
      ...
      className={
        useFallback ? "h-full w-full object-contain p-2 bg-black/40" : className
      }
    />
  );
}
...
export default function SchedulePage() {
  ...
  const groupedByDay = useMemo(() => {
    ...
    for (const item of items) {
      ...
      if (!item.url.includes("youtube.com") && !item.url.includes("twitch.tv"))
        continue;
      const shiftedAt = item.publishedAt + shiftMs;
      const d = new Date(shiftedAt);
      const startHour =
        normalizeHourForTimeline(d.getHours()) + d.getMinutes() / 60;
  ...
  const timeline = useMemo(
    () => buildTimelineConfig(groupedByDay),
    [groupedByDay],
  );
  ...
  return (
    <div className="w-full max-w-5xl mx-auto px-4 py-16">
      ...
      <div className="liquid-glass-card bg-[#060b1c]/80 backdrop-blur-md rounded-lg border border-primary/30 p-6 md:p-8 shadow-xl shadow-primary/10">
              ...
              {loading ? (
                ...
                <div>
                  {groupedByDay.map((group) => {
                    const rowLabel = formatTimelineRowLabel(
                      group.date,
                      language,
                    );
                    return (
                        ...
                        <div className="relative px-3 py-3">
                          ...
                          >
                            {timeline.columns.map((hour) => (
                              <div
                                key={hour}
                                className="border-l border-primary/10 first:border-l-0"
                              />
                            ))}
                          </div>
                          ...
                          <div className="relative space-y-2">
                            {group.videos.map((video) => {
                              const leftPercent = calcTimelineLeftPercent(
                                video.startHour,
                                timeline,
                              );
                              const widthPercent = calcTimelineWidthPercent(
                                video.durationSeconds,
                                timeline,
                              );
                              const clampedLeft = Math.min(
                                Math.max(leftPercent, 0),
                                96,
                              );
                              const clampedWidth = Math.min(
                                Math.max(widthPercent, 8),
                                75,
                              );
                              const locale = timelineLocaleByLang[language];
